...
\begin{abstract}
      \noindent
      % rivedere definizione di effetto rashomon, anche nelle altre parti in cui viene citato
...
Quando ci affidiamo ai metodi di spiegazione per modelli di \emph{machine
      learning}, emerge una domanda: \textit{fissato un metodo di spiegazione, quanto
      sono \textbf{stabili} (cfr.\ sez.~\ref{sec:stabilita}) le spiegazioni al
      variare del modello all’interno del \emph{Rashomon set}, e come possiamo
      quantificarne la \textbf{variabilità}?} La sola accuratezza non basta a
caratterizzare un sistema di ML: spesso coesistono più modelli con prestazioni
equivalenti ma comportamenti interni differenti, un fenomeno noto come
\emph{model multiplicity} \citep{black2022modelmultiplicity}. Tale molteplicità
può riguardare sia le predizioni (\emph{predictive multiplicity}) sia i
“processi decisionali” sottostanti (\emph{procedural multiplicity}), di cui le
spiegazioni XAI offrono una finestra osservativa.

In questo quadro, viene adottato un \emph{Rashomon set} per isolare l’effetto
delle scelte di modello a parità di performance e porre la questione centrale
del \emph{Rashomon effect} \citep{mueller2023rashomon,leventi2023consistency}:
se due modelli raggiungono lo stesso livello di accuratezza, possiamo
aspettarci che “spieghino” le proprie decisioni nello stesso modo? La risposta
a questa domanda si basa sulla misurazione di stabilità e variabilità delle
spiegazioni con metriche quantitative e valutazioni di fedeltà.
...
\begin{enumerate}
      ...
      \item Applicare più metriche di similarità (ad esempio \emph{Structural Similarity
                  Index} — SSIM, correlazione di Pearson, correlazione di Spearman, similarità
            coseno, MAE) alle spiegazioni prodotte.
      \item Confrontare i risultati per capire come varia la spiegazione in funzione sia
            del modello che del metodo scelto, distinguendo tra scenario
            \emph{intra-modello} (metodi diversi, stesso modello) e \emph{inter-modello}
            (stesso metodo, modelli diversi).
\end{enumerate}
...
\begin{itemize}
      \item Due metodi di spiegazione, applicati allo stesso input, possono evidenziare
            regioni o feature completamente diverse come rilevanti (variabilità
            \emph{intra-modello}).
      \item Metriche diverse possono portare a conclusioni contrastanti sul grado di
            similarità (diverse nozioni di “somiglianza”).
...
\emph{fedele}; al contrario, un impatto minimo sulla predizione indicherebbe
una spiegazione poco utile. La qualità viene riassunta tramite \emph{Area Over
...
prospettive complementari. In questo lavoro si adotta quindi una doppia
prospettiva:
\begin{itemize}
      \item analisi della \textbf{similarità/stabilità} tra spiegazioni, sia
            \emph{intra-modello} sia \emph{inter-modello} (sez.~\ref{sec:stabilita});
      \item analisi della \textbf{fedeltà} tramite MoRF e \emph{Area Over the Perturbation
                  Curve} (AOPC).
\end{itemize}

Questa impostazione rende esplicito come la \textbf{model multiplicity}
\cite{black2022modelmultiplicity} possa manifestarsi non solo nelle predizioni,
ma anche nelle \textit{spiegazioni}: modelli equivalenti in accuratezza possono
produrre attribuzioni tra loro differenti.
...
% rivedere questa parte, non mi convinvce
...
\textbf{validation accuracy} del checkpoint migliore. In linea con le
raccomandazioni sulla trasparenza nella scelta dei modelli in presenza di
\emph{model multiplicity} \citep{black2022modelmultiplicity}, rendiamo
esplicita la \emph{meta-regola} di selezione; si includono nel \emph{Rashomon
      set} tutti i modelli con \emph{validation accuracy} entro l’1\% dal migliore,
ovvero
\[
...
\begin{itemize}
      ...
      \item \textbf{Equità nel confronto}: applicando la stessa regola a tutti i modelli, ciascun membro del Rashomon set viene selezionato nelle condizioni di miglior equilibrio tra bias e varianza, come raccomandato nelle buone pratiche di confronto tra modelli.
            ...
            \begin{comment}
            \item \textbf{Comparabilità tra modelli}: applicando lo \emph{stesso} criterio a tutti i seed
            (salvataggio del \emph{best checkpoint} su \emph{validation} e \texttt{patience} uniforme),
            ogni modello è confrontato nel proprio \emph{punto ottimale in validation}. Il confronto
            non dipende da un numero di epoche arbitrario ma da una regola uniforme e riproducibile,
            separata dalla valutazione finale su \emph{test}.
            \end{comment}
...
% forse è meglio farne una discussione più disccorsiva invece di una sezione con una tabella
...
Il confronto viene condotto calcolando, per ogni immagine del \textit{test
      set}, la similarità tra mappe di attribuzione. Il focus principale è lo
scenario \textbf{inter-modello}, mentre lo scenario \textbf{intra-modello} è
usato come analisi di controllo.

\paragraph{Inter-modello (focus).}
Fissato un metodo di spiegazione $a$ e un insieme di modelli $\mathcal{M}$
appartenenti al \emph{Rashomon set}, misuriamo la stabilità del metodo rispetto
a variazioni nei parametri del modello: per ogni immagine $x$, confrontiamo le
mappe $E_a(x;m_i)$ e $E_a(x;m_j)$ per tutte le coppie $i<j$, mantenendo
costanti dati e pipeline di spiegazione.

\paragraph{Intra-modello (controllo).}
Per completezza, confrontiamo metodi diversi applicati allo \emph{stesso}
modello. Questo scenario quantifica la coerenza tra approcci XAI differenti
quando il modello è fissato.

\medskip
Le attribuzioni sono calcolate rispetto alla \textbf{classe vera} (\textit{true label})\footnote{La \emph{true label} è l’etichetta corretta associata al campione. L’uso costante della true label evita che errori di classificazione contaminino il confronto tra mappe; resta tuttavia una potenziale minaccia alla validità esterna in contesti reali in cui la classe predetta sia l’unico riferimento disponibile.}.

\paragraph{Collegamento alla \emph{model multiplicity}.}
In modo simile a \citet{black2022modelmultiplicity}, che misura la dissonanza tra modelli come \emph{disagreement} atteso sulle predizioni, qui viene quantificata la \textbf{stabilità delle spiegazioni} come \emph{similarità attesa} tra mappe di attribuzione prodotte dallo stesso metodo XAI su modelli diversi del Rashomon set. In altre parole, studiamo la “molteplicità procedurale” a livello di spiegazione, mantenendo costanti dati e accuratezza.

\subsection{Definizione operativa di stabilità} \label{sec:stabilita}
...
\emph{stabilità inter-modello}: fissato un metodo $a$ e una famiglia di modelli
$\mathcal{M}$ con prestazioni equivalenti, le spiegazioni $E_a(x;m)$ restano
